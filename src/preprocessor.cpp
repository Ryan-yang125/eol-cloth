	...
	for (int i = 0; i < collisions.size(); i++)
	{
		...
		else if (collisions[i]->count1 == 1 && collisions[i]->count2 == 3)
		{
			if ((mesh.nodes[collisions[i]->verts2(0)]->EoL && mesh.nodes[collisions[i]->verts2(0)]->cornerID == collisions[i]->verts1(0)) ||
				...
				(mesh.nodes[collisions[i]->verts2(2)]->EoL && mesh.nodes[collisions[i]->verts2(2)]->cornerID == collisions[i]->verts1(0)))
				continue; // If the triangle's point touches obstacle's vert

			Vert *v0 = mesh.verts[collisions[i]->verts2(0)],
				 *v1 = mesh.verts[collisions[i]->verts2(1)],
				 *v2 = mesh.verts[collisions[i]->verts2(2)];

			// The coordinate of collision point
			double yX = collisions[i]->weights2(0) * v0->u[1] + collisions[i]->weights2(1) * v1->u[1] + collisions[i]->weights2(2) * v2->u[1];
			double xX = collisions[i]->weights2(0) * v0->u[0] + collisions[i]->weights2(1) * v1->u[0] + collisions[i]->weights2(2) * v2->u[0];

			// Boundary
			if (onBoundaryA(boundary, Vector3d(xX, yX, 0.0)))
				continue; // If the point is on boundary, just skip
			...
			for (int j = 0; j < 3; j++)
			{
				if (barycentric[j] < threshold)
					on_edge = true; // The collision point is exactly on the boundary
			}
		...
		{
			if (mesh.nodes[collisions[i]->verts2(0)]->EoL || mesh.nodes[collisions[i]->verts2(1)]->EoL)
				continue; // If the collision point is exactly on the end
			...
			if (onBoundaryB(boundary, Vector3d(xX, yX, 0.0), Vec3(e2(0), e2(1), 0.0)))
				continue;
...
void Preprocessor::markPreserve(Mesh &mesh)
{
	for (int i = 0; i < mesh.edges.size(); i++)
	{
		...
		{
			if (n0->cornerID >= 0 && n1->cornerID >= 0)
				continue; //Just for point collision case
...
void Preprocessor::cleanMesh(Mesh &mesh, const Eigen::MatrixXd &boundary)
{
	...
	while (!allclear)
	{
		...
		while (!allclear)
		{
			iter++;
			if (iter == 3)
				break; // in case be trapped in infinite loop
			allclear = true;
